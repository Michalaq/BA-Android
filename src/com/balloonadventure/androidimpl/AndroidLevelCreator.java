...
public class AndroidLevelCreator extends BalloonAdventureLevelCreator {
	...
	static {
		try {
			map.put("orbit", OrbitTransformation.class.getConstructor(float.class, float.class, float.class));
			map.put("box", ElasticCollisionBoxTransformation.class.getConstructor(float.class, float.class, float.class, float.class, float.class, float.class));
			map.put("velocity", VelocityTransformation.class.getConstructor(float.class, float.class));
			map.put("orbit_cloud", OrbitTransformation.class.getConstructor(SpriteObject.class, float.class));
			map.put("pulse", PulseTransformation.class.getConstructor(float.class, float.class, float.class));
			map.put("move", MoveTransformation.class.getConstructor(float.class, float.class, float.class, float.class, float.class, float.class));
	 ...
	 * 		"bgScenery":"background_scenery_name",
	 * 		"clouds": [
	 * 			{
	 * 				"cloud_type":"spiky",
	 * 				"x":#x,
	...
	private class TransformationData {
		...
		public String toString() {
			return "[type: " + trans_type + " values: " + Arrays.toString(values) + "]";
		}
		
		public Transformation<SpriteObject> makeTransformation() {
			try {
				return (Transformation<SpriteObject>) map.get(trans_type).newInstance(getNormalizedValues(values.clone()));
		...
		private Object[] getNormalizedValues(float values[]) {
			GameEngine.sendDebugMessage("TRANS", "Value0: " + String.valueOf(values[0]));
			...
			if (trans_type.equals("orbit"))
				return new Object[] {normalizeX(values[0]), normalizeY(values[1]), values[2]};
			
			if (trans_type.equals("velocity"))
				return new Object[] { normalizeX(values[0]), normalizeY(values[1]) };
	...
	private class FieldData {
		...
		@Override
		public String toString() {
			return String.format("[type: %s, x: %f, y: %f, radius: %f]", field_type, x, y, radius);
		}
	}
	private class RawLevelData {
		...
		@Override
		public String toString() { return String.format("id: %d, layerCount: %d, bgScenery: %s, clouds: %s, field: %s",
														id, layerCount, bgScenery.toString(), clouds.toString(), field.toString()); }
	}
	private class GlobalLevelConfig {
		private int levelCount;
	}
	
	public AndroidLevelCreator(Context context, ClickSource cs) {
		super(cs);
	...
	private BalloonAdventureLevelData createDataFromRaw(RawLevelData rld) {
		...
		for(CloudData cd : rld.clouds) {
			c = WorldObjectFactory.createCloud(CloudType.valueOf(cd.cloud_type),
												normalizeX(cd.x), normalizeY(cd.y)+64);
			...
			clouds.add(c);
			//c.addTransformation(new OrbitTransformation(GraphicsDrawer.getDefaultScreenWidth()/2,
			//					GraphicsDrawer.getDefaultScreenHeight()/2, 180));
			//c.addTransformation(new ElasticCollisionBoxTransformation(normalizeX(cd.x), normalizeY(cd.y)+64, normalizeX(cd.rangeX), normalizeY(cd.rangeY), vX, vY));
		}
		InLevelBalloonManager bm = new RandomBalloonManager(3, new BalloonType[] {BalloonType.BALLOON_GOLD, BalloonType.BALLOON_BLUE, BalloonType.BALLOON_RED}, new float[] {0.05f, 0.2f});
		BackgroundScenery bs = BackgroundObjectFactory.createBackgroundScenery(String.valueOf(rld.bgScenery));
		bs.addTransformation(new ShiftVelocityTransformation(2, 0));
		Field f = null;
		if (rld.field.field_type.equals("circular")) {
			f = BackgroundObjectFactory.createCircularField(String.valueOf(rld.field.texture), normalizeX(rld.field.x), normalizeY(rld.field.y), normalizeX(rld.field.radius));
		} else if (rld.field.field_type.equals("rectangular")) {
			f = BackgroundObjectFactory.createRectangularField(String.valueOf(rld.field.texture), normalizeX(rld.field.x), normalizeY(rld.field.y),normalizeX(rld.field.xEnd), normalizeY(rld.field.yEnd));
		} //asdasdasd
		return new BalloonAdventureLevelData(rld.layerCount, rld.id, rld.timeLeft, bs, clouds, f, bm);
	}
	
	private String getJSONFromFile(String fileName) throws IOException {
		InputStream is;
		is = context.getResources().openRawResource(context.getResources().
				getIdentifier(fileName,//+bitmapExtension,
	...
	private String getJSONFromExternalFile(String fileName) {
		...
		try {
			FileInputStream fis;
			File f = new File(context.getExternalFilesDir(null), fileName+".json");
	...
	public void loadLevelData() {
		try {
			...
			GlobalLevelConfig clg = g.fromJson(configString, GlobalLevelConfig.class);
			...
			levelData = new BalloonAdventureLevelData[levelCount+additionalLevels+1];
			String jsonString;
			for(int i=1; i<=levelCount; i++) {
				jsonString = getJSONFromFile(levelFilePrefix + String.valueOf(i));
			...
			for(int i=1; i<=additionalLevels; i++) {
				jsonString = getJSONFromExternalFile(levelFilePrefix + String.valueOf(i));
				RawLevelData rld = g.fromJson(jsonString, RawLevelData.class);
				rld.id += levelCount;
